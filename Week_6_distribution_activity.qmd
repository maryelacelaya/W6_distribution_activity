...
Get in a group of \~4 and select one or more data collection activities. Collect data for approximately 10 minutes (aim for at least 20 data points as a team) and then find a way to get your data into R.
...
#First I introduced the data;I avoided the word "length" because there are functions named like that
Leaf<-c(1:20)
Size<-c(10.8,11.7,11.5,8.5,10.5,6.4,6,7.9,7.6,2.9,2.7,3,7.9,11.8,11.5,6.9,16.8,14.3,10.4,9.9)
Leaf<-as.numeric(Leaf)



#To create a data frame
Leaves<-data.frame(Leaf,Size)
print.data.frame(Leaves)

#Plot the data
plot(Size, Leaf)

breaks <- seq(min(Size), max(Size), by = 1) 
...
axis(1, at = seq(min(Size), max(Size), by = 1), las = 1)
...
1.  What is the general shape of the data? Looks like normal distribution
2.  Are the data bounded (e.g., percents, non-negative)? They are positive, greater than 0,
3.  Are the data continuous, categorical, binary, or what? Numeric, continuous,
...
#to simulate 20 data points
sim20 <-rnorm(20,mean=8.95,sd=3.716747)

#to plot the freq distribution of the simulated data
plot(sim20)
hist(sim20,
     col = "lightgreen")
```

Do the simulated data look like yours? Why or why not? Yes, looks pretty similar
...
# do your calculations here 
#to simulate 20 data points
sim1000 <-rnorm(1000,mean=8.95,sd=3.716747)
...
l=mean(Leaves$Size)
cv=10.8
(l^k)*(exp(-l))/(factorial(cv))

#less common value: 9.9
lc=9.9
(l^k)*(exp(-l))/(factorial(lc))


#not observed value: 50
no=50
(l^k)*(exp(-l))/(factorial(no))
